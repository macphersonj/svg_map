var rsr = Raphael('map', '938.3', '507.4');
...
africa.attr({fill: '#CC3300','stroke-width': '0','stroke-opacity': '1', 'cursor': 'pointer'}).data({'id': 'africa', 'region': 'Africa', 'description': 'Efficiently unleash cross-media information without cross-media value. Quickly maximize timely deliverables for real-time schemas. Dramatically maintain clicks-and-mortar solutions without functional solutions.'});
regions.push(africa);
...
south_america.attr({fill: '#CC3300','stroke-width': '0','stroke-opacity': '1', 'cursor': 'pointer'}).data({'id': 'south_america', 'region': 'South America', 'description': 'Completely synergize resource sucking relationships via premier niche markets. Professionally cultivate one-to-one customer service with robust ideas. Dynamically innovate resource-leveling customer service for state of the art customer service'});
...
asia_pacific.attr({fill: '#CC3300','stroke-width': '0','stroke-opacity': '1', 'cursor': 'pointer'}).data({'id': 'asia_pacific', 'region': 'Asia Pacific', 'description': 'Phosfluorescently engage worldwide methodologies with web-enabled technology. Interactively coordinate proactive e-commerce via process-centric "outside the box" thinking. Completely pursue scalable customer service through sustainable potentialities.'});
...
europe.attr({fill: '#CC3300','stroke-width': '0','stroke-opacity': '1', 'cursor': 'pointer'}).data({'id': 'europe', 'region': 'Europe', 'description': 'Collaboratively administrate turnkey channels whereas virtual e-tailers. Objectively seize scalable metrics whereas proactive e-services. Seamlessly empower fully researched growth strategies and interoperable internal or "organic" sources.'});
regions.push(europe);
  ...
  regions[i].mouseover(function(e){
	...
	regions[i].mousedown(function(e){
		...
		if (typeof e !== 'undefined') {
			posx = e.pageX - $map.offset().left - 120;
			posy = e.pageY - $map.offset().top - -45;
		} else {
